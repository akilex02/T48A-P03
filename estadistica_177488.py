...
Original file is located at
    https://colab.research.google.com/drive/1pQ0SsIhnrdjb1m-u6SZh6mDNwk0dZCV4
"""
...
media = np.mean(datos_mexico, axis=0)
print(f"Media de altura y cintura: {media}")

# Desviación estándar por columnas
desv = np.std(datos_mexico, axis=0)
print(f"Desviación estándar de altura y cintura: {desv}")

# Varianza por filas
varianza = np.var(data_2d, axis=0)
print(f"Varianza de altura y cintura: {varianza}")

# Máximo y mínimo por columnas
maxi = np.max(datos_mexico, axis=0)
mini = np.min(datos_mexico, axis=0)
print(f"Máximo por columnas: {maxi}, Mínimo por columnas: {mini}")

# Percentiles por filas (percentil 50 = mediana)
percentil50 = np.percentile(datos_mexico, 50, axis=0)
print(f"Percentil 50 (mediana) por columnas: {percentil50}")

# Correlacion entre columnas (primera y segunda columna)
correlacion = np.corrcoef(datos_mexico[:, 0], datos_mexico[:, 1])
print(f"Correlación entre la primera y segunda columna: \n{correlacion}")

# Histograma (requiere matplotlib)

import matplotlib.pyplot as plt
plt.hist(altura, bins=15)  # bins define el número de barras en el histograma
plt.xlabel("Valor")
plt.ylabel("Frecuencia")
plt.title("Histograma de altura")
plt.show()

plt.hist(cintura, bins=15)  # bins define el número de barras en el histograma
plt.xlabel("Valor")
plt.ylabel("Frecuencia")
